// Ref: https://codepen.io/stefanweck/pen/ZLpgBa
/**
 * Constants
 */
...
class Application {
     ...
     */
    constructor() {
        this.canvas = document.getElementById("background_img");
        this.cathead = document.getElementById("background_cat_head");
        ...
        this.cats = [];
        this.numberOfCats = Math.round((this.width + this.height) / 200);
    ...
    initializeCats() {
        for (let i = 0; i < this.numberOfCats; i++) {
            let position = new Vector2D(getRandomInt(0, this.width), getRandomInt(0, this.height));
            while (this.detectColision(position)) {
    ...
    update() {
        for (let i = 0; i < this.cats.length; i++) {
            this.cats[i].update(this.mousePosition);
        }
    }
    ...
    render() {
        ...
        for (let i = 0; i < this.cats.length; i++) {
            this.cats[i].render(this.context);
        }
    }
...
class Ghost {
    ...
    /**
     * Create two eyes for this cat! cats can't see without their eyes
     * @return void
     */
    initialize() {
        this.eyes.push(new Eye(this.position.getX() - this.eyeDistance, this.position.getY() - 23));
        this.eyes.push(new Eye(this.position.getX() + this.eyeDistance, this.position.getY() - 23));
        this.eyesWhite.push(new EyeWhite(this.position.getX() - this.eyeDistance, this.position.getY() - 23));
        this.eyesWhite.push(new EyeWhite(this.position.getX() + this.eyeDistance, this.position.getY() - 23));
    }
    ...
    update(mousePosition) {
        //Randomize the velocity of this cat
        if (Math.random() < 0.01) {
            this.velocity.setLength(Math.random() * 2 + 1);
            this.velocity.setAngle(Math.random() * TWO_PI);
        }
        ...
        for (let i = 0; i < this.eyes.length; i++) {
            ...
            this.eyesWhite[i].update(bodyBounce, angle);
        }
    ...
    render() {
        ...
        this.context.drawImage(this.cathead, this.position.getX() - this.radius, this.position.getY() - this.radius, this.radius * 2, this.radius * 2);
...
window.onload = function () {
    ...
    //Initialize the cats objects
    application.initializeCats();
