...
class GestureUnlock {
    ...
    draw(x, y) {
        ...
        this.drawLine(x-this.offsetX, y-this.offsetY);
        this.drawCircle();
    }
    ...
    indexOfPoint(x,y) {
        ...
        for(let i=0;i<this.pointPos.length;i++) {
            if((Math.abs(x - this.pointPos[i][0] - this.offsetX) < this.circleR) && 
                (Math.abs(y - this.pointPos[i][1] - this.offsetY) < this.circleR)) {
                return i;
            }
        }
        return -1;
    ...
    touchEnd(e) {
        ...
        if(this.points.length > 1) {
            ...
            */
            ...
            console.log("点击的顺序是：" + this.points.join(" -> "));
            update_gesture_login(this.points);
            this.points = [];
            this.draw();
        }
    }
...
function update_gesture_login(gesture_array){
    $.ajax({
            type: "POST",
            data:{
                "gesture_array": JSON.stringify(gesture_array)
            },
            url: '/auth/login/gesture/update',
...
$(document).ready(function () {
    ...
    $('#profile_basic_tab').toggleClass('active');
    $('#ip_content').css('display','none');
    $('#gesture_content').css('display','none');

    $('#profile_basic_tab').click(function() {
        $(this).toggleClass('active');

        $('#profile_ip_tab').removeClass('active');
        $('#profile_gesture_tab').removeClass('active');
    ...
    $('#profile_ip_tab').click(function() {
        $(this).toggleClass('active');

        $('#profile_basic_tab').removeClass('active');
        $('#profile_gesture_tab').removeClass('active');

        $('#basic_content').css('display','none');
        $('#gesture_content').css('display','none');

        $('#ip_content').css('display','block');
	});
    
    $('#profile_gesture_tab').click(function() {
        $(this).toggleClass('active');
        ...
        $('#gesture_content').css('display','block');
        var gestureUnlock = new GestureUnlock(canvas);
	});
});
