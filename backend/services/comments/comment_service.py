from backend.models.comments.comment import CommentModel
...
class CommentService(BaseService):
    ...
    def get_all_video_comment(self):
        with session_scope() as session:
            return session.query(self.model).filter(self.model.category == 'video').filter(self.model.is_removed.isnot(True)).order_by(self.model.created_at.desc()).all()

    def deactive_comment(self, id):
        with session_scope() as session:
            comment = session.query(self.model).filter(
