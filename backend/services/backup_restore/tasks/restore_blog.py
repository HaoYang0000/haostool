...
class RestoreBlogTask(Task):
    ...
    def run(self):
        try:
            ...
            self._pre_check()
            backup_result = self._restore_blog_content()
    ...
    def _restore_blog_content(self):
        logger.info(f"Restore blog content...")
        target_dir = f'{UPLOAD_ROOT}/{BLOG_IMAGE_DIR}/'
        source_dir = f'{UPLOAD_ROOT}/{BACKUP_DIR}/{self.backup_id}/{BLOG_IMAGE_DIR}/'
